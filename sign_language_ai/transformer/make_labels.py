...
# 定義特徵資料存放的 Windows 絕對路徑
# 使用 r"..." 代表原始字串 (Raw String)，能防止反斜線 \ 被誤判為跳脫字元
data_dir = r"E:\HandSignProject\LSTM\data"
# 建立一個空清單，用來儲存所有資料列（每筆資料包含檔案路徑與標籤）
rows = []

# 第一層迴圈：遍歷指定絕對路徑資料夾底下的所有子項目（各個手語詞彙的標籤資料夾）
for label in os.listdir(data_dir):
    # 組合出子項目的完整路徑（例如：E:\HandSignProject\LSTM\data\謝謝_A）
    ...
    for f in os.listdir(label_dir):
        ...
        # 💡 使用正則表達式（re.sub）進行精準的標籤清洗
        # r'_[A-Za-z]$' 中的 $ 代表字串結尾。
        # 這意味著只有當「底線+單個英文字母」出現在資料夾名稱的最末尾時（例如：_A、_B）才會被刪除，
        # 能有效避免誤刪資料夾名稱中間夾雜的類似字元，確保標籤轉換更安全。
