...
while cap.isOpened():
    ...
    if hres.multi_hand_landmarks and hres.multi_handedness:
        ...
        for idx, hand_lms in enumerate(hres.multi_hand_landmarks):
            ...
            angles = hand_angles(hand_lms.landmark, img_w, img_h) # ✅ 傳入寬高
    ...
    # ✅ Rolling Window (保持最新的 SEQ_LEN 幀)
    ...
    if hand_exist and len(buffer) == SEQ_LEN:
        ...
        buffer = [] # 手不見了就清空 Buffer，避免殘留畫面干擾
        display_text = "請做手勢"
